...
(function(){
    ...
    var prevalue;
    var searchT;
    $('#course').on('keyup',search);
    $('#course').on('blur',()=>{
        if (!$('#course').val())
        {
            $(".display-search-course").html("");
        $(".display-search-course").css('display','none');
        $('#submitSearch').html('إبحث');
        }
    });
    function search ()
    {
        if ($('#course').val() != prevalue)
    {
        clearSearch();
        ...
        if (!spinner)
                    {
                        $('#submitSearch').html('<div class="lds-ripple"><div></div><div></div></div>');
                        spinner = true;
                    }
                     searchT = setTimeout(function(){
                        $(".display-search-course").css('display','block');
                        getSearch($('#course').val());
                       console.log("hahah");
                        spinner = false;
                        $('#submitSearch').html("إبحث");
                        },1000); 
...
(function(){
    ...
    $('#submit').on('click',function(){
			...
			$.ajax({
				...
				{
					$('.log-in-footer').html("<b style='color:red'>معلومات خاطئة</b>");
				}
				}
			});
	});
